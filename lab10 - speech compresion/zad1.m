...
for nr=1:Nramek
    ...
    s2=bx;
    %figure;
    %plot(n,s2);title('fragment sygnalu mowy przed progowaniem');
    P=0.3*max(bx);
    for i=1:240
        if bx(i)>=P
            bx(i)=bx(i)-P;
    ...
    if(rmax>0.35*r(1)) 
        ...
        d=d+1;
        dzwiek='Dzwieczny';
        %dzwieczny_wyswietlony = true;
    else
        T=0;
        dzwiek='Bezdziweczny';
        %bezdzwieczny_wyswietlony = true;
    end %gloska dzwieczna/bezdziweczna?

    if(T>80) T=round(T/2); end %znaleziona druga podharmoniczna
   ...
   % hold off;
    ...
    % usun pierwszy znak "%" i ustaw: T = 80, 50, 30, 0 (w celach testowych)
